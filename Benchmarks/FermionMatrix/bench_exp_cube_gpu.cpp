...
void bench_exp_cube_M(int nt, int nx){
    ...
    std::string filename;
    filename = "results_gpu/results_bench_exp_cube_gpu_M_nx_" + std::to_string(nx) + ".txt";  
    std::ofstream fout; 
...
void bench_exp_cube_MdaggerM(int nt, int nx){
    ...
    std::cout<< "Estimated bandwidth: "<<est_bw<<std::endl;
    std::string filename;
    filename = "results_gpu/results_bench_exp_cube_gpu_MdaggerM_nx_" + std::to_string(nx) + ".txt";  
    std::ofstream fout; 
    fout.open(filename , std::ios::out | std::ios::app);
    fout<<nt<<" "<<nx<<" "<<timings<<" "<<est_bw<<" "<<est_err<<std::endl;
    fout.close();
...
void bench_exp_cube_logDetM(int nt, int nx){
    ...
    std::cout<< "Estimated bandwidth: "<<est_bw<<std::endl;
    std::string filename;
    filename = "results_gpu/results_bench_exp_cube_gpu_logDetM_nx_" + std::to_string(nx) + ".txt";  
    std::ofstream fout; 
    fout.open(filename , std::ios::out | std::ios::app);
    fout<<nt<<" "<<nx<<" "<<timings<<" "<<est_bw<<" "<<est_err<<std::endl;
    fout.close();
