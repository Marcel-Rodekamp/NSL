...
void bench_exp_square_M(int nt, int nx){
    ...
    std::string filename;
    filename = "results_gpu/results_bench_exp_square_gpu_M_nx_" + std::to_string(nx) + ".txt";   
    std::ofstream fout; 
...
void bench_exp_square_MdaggerM(int nt, int nx){
    std::vector<std::size_t> n = {nx,nx};
    NSL::Tensor<NSL::complex<double>> psi(NSL::GPU(),nt, nx*nx), phi(NSL::GPU(),nt, nx*nx);
    phi.rand();
    psi.rand();
    ...
    std::cout<< "Estimated bandwidth: "<<est_bw<<std::endl;
    std::string filename;
    filename = "results_gpu/results_bench_exp_square_gpu_MdaggerM_nx_" + std::to_string(nx) + ".txt";   
    std::ofstream fout; 
    fout.open(filename , std::ios::out | std::ios::app);
    fout<<nt<<" "<<nx<<" "<<timings<<" "<<est_bw<<std::endl;
    fout.close();
...
void bench_exp_square_logDetM(int nt, int nx){
    std::vector<std::size_t> n = {nx,nx};
    NSL::Tensor<NSL::complex<double>> psi(NSL::GPU(),nt, nx*nx), phi(NSL::GPU(),nt, nx*nx);
    phi.rand();
    psi.rand();
    ...
    std::cout<< "Estimated bandwidth: "<<est_bw<<std::endl;
    std::string filename;
    filename = "results_gpu/results_bench_exp_square_gpu_logDetM_nx_" + std::to_string(nx) + ".txt";    
    std::ofstream fout; 
    fout.open(filename , std::ios::out | std::ios::app);
    fout<<nt<<" "<<nx<<" "<<timings<<" "<<est_bw<<std::endl;
    fout.close();
...
int main(){
    ...
    for(int i=0; i< Nt.size(); i++){
            ...
            bench_exp_square_M(Nt[i],8);
        }
    
    for(int i=0; i< Nt.size(); i++){
        
            bench_exp_square_MdaggerM(Nt[i],8);
        }
    
    for(int i=0; i< Nt.size(); i++){
        
            bench_exp_square_logDetM(Nt[i],8);
        }
