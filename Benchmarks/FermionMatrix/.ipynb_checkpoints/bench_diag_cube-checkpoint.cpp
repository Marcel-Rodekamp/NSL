...
void bench_diag_cube_M(int nt, int nx){
    ...
    std::string filename;
    filename = "results/results_bench_diag_cube_cpu_M_nx_" + std::to_string(nx) + ".txt";  
    std::ofstream fout; 
...
void bench_diag_cube_MdaggerM(int nt, int nx){
    ...
    std::cout<< "Estimated bandwidth: "<<est_bw<<std::endl;
    std::string filename;
    filename = "results/results_bench_diag_cube_cpu_MdaggerM_nx_" + std::to_string(nx) + ".txt";  
    std::ofstream fout; 
    fout.open(filename , std::ios::out | std::ios::app);
    fout<<nt<<" "<<nx<<" "<<timings<<" "<<est_bw<<" "<<est_err<<std::endl;
    fout.close();
...
void bench_diag_cube_logDetM(int nt, int nx){
    ...
    std::cout<< "Estimated bandwidth: "<<est_bw<<std::endl;
    std::string filename;
    filename = "results/results_bench_diag_cube_cpu_logDetM_nx_" + std::to_string(nx) + ".txt";  
    std::ofstream fout; 
    fout.open(filename , std::ios::out | std::ios::app);
    fout<<nt<<" "<<nx<<" "<<timings<<" "<<est_bw<<" "<<est_err<<std::endl;
    fout.close();
