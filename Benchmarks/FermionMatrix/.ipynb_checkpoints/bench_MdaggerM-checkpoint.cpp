...
int main(){
    ...
    //Memory in GB
    const double mem_GB  = 2*(3 * (2*nt*nx * sizeof(NSL::complex<double>)) + nt*nx*(nx) * sizeof(NSL::complex<double>)) * pow(10,-9) //M and Mdagger
     + (8 * (2*nt*nx * sizeof(NSL::complex<double>)) + 4*nt*nx*(nx) * sizeof(NSL::complex<double>)) * pow(10,-9); //9.3 * pow(10,-10);
