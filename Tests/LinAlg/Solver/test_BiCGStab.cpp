...
/*! \file test_BiCGStab.hpp
 *
 * This file tests the the application of BiCGStab  
 * 
 * As the BiCGStab works on symetric & positive definite matrices these tests only
 ...
 * a big issue in the BiCGStab algorithm.
 * */
...
COMPLEX_NSL_TEST_CASE("BiCGStab - Hubbard Exp - Complete", "[BiCGStab,Hubbard Exp,Complete]"){
    NSL::size_t Nt = GENERATE(8,32);
    ...
    M.populate(phi);
    test_BiCGStab_fermionMatrix<TestType>(M,eps,Nt,Nx);

}

COMPLEX_NSL_TEST_CASE("BiCGStab - Hubbard Exp - Ring", "[BiCGStab,Hubbard Exp,Ring]"){
    NSL::size_t Nt = GENERATE(8,32);
    NSL::size_t Nx = GENERATE(2,8);
    TestType beta = GENERATE(1,1.5);
...
COMPLEX_NSL_TEST_CASE("BiCGStab - Hubbard Exp - Square 1D", "[BiCGStab,Hubbard Exp,Square,1D]"){
    NSL::size_t Nt = GENERATE(8,32);
    NSL::size_t Nx = GENERATE(2,8);
    TestType beta = GENERATE(1,1.5);
...
COMPLEX_NSL_TEST_CASE("BiCGStab - Hubbard Exp - Square 2D", "[BiCGStab,Hubbard Exp,Square,2D]"){
    NSL::size_t Nt = GENERATE(8,32);
    NSL::size_t Nx1 = GENERATE(2,3);
    NSL::size_t Nx2 = GENERATE(2,3);
...
COMPLEX_NSL_TEST_CASE("BiCGStab - Hubbard Exp - Square 3D", "[BiCGStab,Hubbard Exp,Square,3D]"){
    NSL::size_t Nt = GENERATE(8,32);
    NSL::size_t Nx1 = GENERATE(2,3);
    NSL::size_t Nx2 = GENERATE(2,3);
    NSL::size_t Nx3 = GENERATE(2,3);
...
COMPLEX_NSL_TEST_CASE("BiCGStab - Hubbard Exp - Square 4D", "[BiCGStab,Hubbard Exp,Square,4D]"){
    NSL::size_t Nt = GENERATE(8,32);
    NSL::size_t Nx1 = GENERATE(2,3);
    NSL::size_t Nx2 = GENERATE(2,3);
    NSL::size_t Nx3 = GENERATE(2,3);
    NSL::size_t Nx4 = GENERATE(2,3);
...
// ======================================================================
// Implementation details: test_BiCGStab_randomMatrix
// ======================================================================
...
void test_BiCGStab_randomMatrix(const typename NSL::RT_extractor<Type>::type eps, NSL::size_t V){
    ...
    NSL::LinAlg::BiCGStab<Type> bicgstab( 
...
void test_BiCGStab_fermionMatrix(FermionMatrix & M,const typename NSL::RT_extractor<Type>::type eps, NSL::size_t Nt, NSL::size_t Nx){
    ...
    NSL::Tensor<Type> b_approx = M.MdaggerM(res);

    INFO( std::string("Nt  = ") + std::to_string(Nt) );
    INFO( std::string("Nx  = ") + std::to_string(Nx) );
    INFO( std::string("eps = ") + std::to_string(eps) );
    INFO( std::string("res = ") + NSL::to_string((b_approx-b).sum()) );

    if(! almost_equal(b_approx,b,getMatchingDigits(eps)).all()){
        WARN( "The BiCGStab converged but M.MdaggerM(x) does not equal b." );
    }
    }
    // MMdagger =========================================================
    {
    ...
    NSL::Tensor<Type> b_approx = M.MMdagger(res);
    ...
    if(! almost_equal(b_approx,b,getMatchingDigits(eps)).all()){
        WARN( "The BiCGStab converged but M.MMdagger(x) does not equal b." );
    }
    }       
