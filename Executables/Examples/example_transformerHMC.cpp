...
struct Net : torch::nn::Module {
    Net(NSL::size_t Nt, NSL::size_t Nx) :
        //shift( {0,-0.06718930700412577} ), // 
        shift( {0,0.06718931277758847} ), // 
        Nt(Nt),
        Nx(Nx)
    {}

    // Implement the Net's algorithm.
    std::pair<torch::Tensor, NSL::complex<double>> forward(torch::Tensor x) {
      // Use one of many tensor manipulation functions.
      return {x + shift, 0.};
    }

    // Implement the Net's algorithm.
    std::pair<torch::Tensor, NSL::complex<double>> inverse(torch::Tensor x) {
      // Use one of many tensor manipulation functions.
      return {torch::real(x) + NSL::complex<double>{0,0}, 0.};
    }
...
int main(int argc, char* argv[]){
    ...
    NSL::TorchTransformer<Type, Net> transformer(
        Net(params["Nt"].to<NSL::size_t>(), lattice.sites())
    );
