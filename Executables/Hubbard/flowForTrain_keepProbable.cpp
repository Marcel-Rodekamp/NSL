...
int main(int argc, char ** argv){
    ...
    if(yml["RK4"]["ImS precision"]){
        ...
        params["ImS precision"] = (params["max flow time"].template to<double>()/params["step size"].template to<double>()) * pow(params["step size"].template to<double>(), 3);
    }
    ...
    NSL::H5IO h5(params["h5file"].template to<std::string>(),NSL::File::Truncate);
    ...
    for(NSL::size_t n = 0; n < params["Nconf"].template to<NSL::size_t>(); ++n){
        ...
        // the "template to<double>()" the whole time.
        ...
        //while(flowTime < params["max flow time"].template to<double>()){
            ...
            if(flowTime >= params["max flow time"].template to<double>()){break;}
            ...
            // the trajectory
            if(std::isnan(NSL::real(actVal)) or std::isnan(NSL::imag(actVal))){
            ...
            if(ImSErr > params["ImS precision"].template to<double>()){
                ...
                // we don't want to enhance tIDX here!
                --tIDX;
            ...
            if(NSL::real(actVal) < targetS){
                ...
                NSL::Logger::info("Configuration {}/{}: flow time {:>.4}(tIDX={}/{}; step size={}) after {} try(s); S = ({:>1.2e}; {:>1.2e}); targetS = {}",
                    n+1,params["Nconf"].template to<NSL::size_t>(),flowTime,tIDX,numMaxFlowStep,stepSize, tries+1, NSL::real(actVal), NSL::imag(actVal), targetS
                );
